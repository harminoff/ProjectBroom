...
static void throwItem(item *theItem, creature *thrower, pos targetLoc, short maxDistance) {
    ...
    for (i=0; i<numCells && i < maxDistance; i++) {
        x = listOfCoordinates[i].x;
        y = listOfCoordinates[i].y;
        brogue_bridge_note_projectile_step(theItem, previousLoc, (pos){ x, y });
        previousLoc = (pos){ x, y };
...
//          }
                if ((theItem->category & WEAPON)
                    ...
                    brogue_bridge_note_projectile_impact(theItem, (pos){ x, y });
                    deleteItem(theItem);
                    return;
                }
                break;
    ...
    brogue_bridge_note_item_landed(theItem, dropLoc);
...
*/
short playerThrowMaxDistance(void) {
    return (short) (12 + 2 * max(rogue.strength - player.weaknessAmount - 12, 2));
}

boolean itemRequiresThrowConfirmation(const item *theItem) {
    return theItem != NULL
        && ((theItem->flags & ITEM_EQUIPPED) || theItem->timesEnchanted > 0)
        && theItem->quantity <= 1;
}
...
boolean throwItemAtTarget(item *theItem, pos zapTarget, boolean recordInput) {
    ...
    if (theItem == NULL
        || !coordinatesAreInMap(zapTarget.x, zapTarget.y)
        || posEq(zapTarget, player.loc)
        || distanceBetween(player.loc, zapTarget) > playerThrowMaxDistance()) {
        return false;
    }
    ...
    if (itemRequiresThrowConfirmation(theItem) && (theItem->flags & ITEM_CURSED)) {
    ...
    *thrownItem = *theItem;
    ...
    brogue_bridge_prepare_thrown_item(theItem, thrownItem, theItem->quantity <= 1);

    throwItem(thrownItem, &player, zapTarget, playerThrowMaxDistance());
...
void throwCommand(item *theItem, boolean autoThrow) {
    ...
    throwItemAtTarget(theItem, zapTarget, true);
}
...
/* The native bridge supplies secondary inventory choices explicitly instead
 * of allowing Brogue's terminal UI to block on promptForItemOfType().  The
 * actual scroll/effect implementation below remains authoritative. */
static boolean bridgeApplySelectionActive = false;
static item *bridgeApplySelectedItem = NULL;
...
boolean itemApplyConfirmationPrompt(item *theItem, char *buffer, size_t bufferSize) {
    ...
    if ((theItem->category & FOOD)
        && STOMACH_SIZE - player.status[STATUS_NUTRITION] < foodTable[theItem->kind].power) {
        snprintf(buffer, bufferSize,
                 "You're not hungry enough to fully enjoy the %s. Eat it anyway?",
    ...
    if (kindTable == NULL
        ...
        || !((theItem->flags & ITEM_MAGIC_DETECTED) || kindTable[theItem->kind].identified)) {
        return false;
    }
    if (kindTable[theItem->kind].identified) {
        snprintf(buffer, bufferSize, "Really %s a %s of %s?",
                 (theItem->category & POTION) ? "drink" : "read",
                 (theItem->category & POTION) ? "potion" : "scroll",
                 kindTable[theItem->kind].name);
    } else {
        snprintf(buffer, bufferSize, "Really %s a cursed %s?",
                 (theItem->category & POTION) ? "drink" : "read",
                 (theItem->category & POTION) ? "potion" : "scroll");
    }
    return true;
...
boolean eat(item *theItem, boolean recordCommands) {
    ...
    if (STOMACH_SIZE - player.status[STATUS_NUTRITION] < foodTable[theItem->kind].power) { // Not hungry enough.
        char buf[COLS * 3];
        itemApplyConfirmationPrompt(theItem, buf, sizeof(buf));
        if (!confirm(buf, false)) {
            return false;
        }
    }
...
void applyItemWithSelection(item *theItem, item *selectedItem) {
    bridgeApplySelectionActive = true;
    bridgeApplySelectedItem = selectedItem;
    apply(theItem);
...
boolean readScroll(item *theItem) {
    ...
    if (itemApplyConfirmationPrompt(theItem, buf, sizeof(buf))) {
        if (!confirm(buf, false)) {
            return false;
        }
    }
    ...
    switch (theItem->kind) {
        case SCROLL_IDENTIFY:
            ...
            if (bridgeApplySelectionActive) {
                theItem = bridgeApplySelectedItem;
                if (theItem == NULL || !(theItem->flags & ITEM_CAN_BE_IDENTIFIED)) {
                    return false;
                }
            } else {
                do {
        ...
        case SCROLL_ENCHANTING:
            ...
            if (bridgeApplySelectionActive) {
                theItem = bridgeApplySelectedItem;
                if (theItem == NULL
                    || !(theItem->category & (WEAPON | ARMOR | RING | STAFF | WAND | CHARM))) {
                    return false;
                }
            } else {
                do {
