...
boolean attack(creature *attacker, creature *defender, boolean lungeAttack) {
    ...
    brogue_bridge_note_attack(attacker, defender, lungeAttack);
...
boolean inflictDamage(creature *attacker, creature *defender,
    ...
    short oldHealth = defender->currentHP;
    ...
    if (defender->currentHP <= damage) { // killed
        ...
        brogue_bridge_note_damage(attacker, defender, oldHealth, true);
        return true;
    } else { // survived
        ...
        if (defender->currentHP < oldHealth) {
            brogue_bridge_note_damage(attacker, defender,
                                      oldHealth - defender->currentHP, false);
        }
    }
...
void killCreature(creature *decedent, boolean administrativeDeath) {
    ...
    brogue_bridge_note_death(decedent, administrativeDeath);
