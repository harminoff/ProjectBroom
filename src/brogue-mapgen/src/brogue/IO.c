...
enum monsterTypes randomAnimateMonster(void) {
...
boolean confirm(char *prompt, boolean alsoDuringPlayback) {
    ...
    /* A live bridge command pauses here and returns Brogue's exact prompt to
     * the frontend. The confirmed retry comes back through this same call. */
    if (brogue_bridge_intercept_confirmation(prompt, &bridgeAnswer)) {
        return bridgeAnswer;
    }

    /* Non-command headless work retains the deterministic autoplay policy. */
    if (rogue.autoPlayingLevel
        || (serverMode && nonInteractivePlayback)
        || (!alsoDuringPlayback && rogue.playbackMode)) {
