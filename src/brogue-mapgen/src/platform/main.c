...
static void printCommandlineHelp() {
    ...
    "--export-dungeon-json PATH --seed SEED --depths N\n"
    "                           export Brogue's generated terrain to deterministic JSON\n"
...
#define TOD(x)  ((double) (x) / FP_FACTOR)
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (strcmp(argv[i], "--export-dungeon-json") == 0) {
            const char *outputPath = NULL;
            uint64_t seed = 0;
            unsigned int depthCount = 0;
            boolean haveSeed = false;
            boolean haveDepthCount = false;
            char errorMessage[ERROR_MESSAGE_LENGTH];

            if (i + 1 >= argc) {
                fprintf(stderr, "--export-dungeon-json requires an output path\n");
                return 1;
            }
            outputPath = argv[++i];
            while (i + 1 < argc) {
                if (strcmp(argv[i + 1], "--seed") == 0 && i + 2 < argc) {
                    if (!tryParseUint64(argv[i + 2], &seed) || seed == 0) {
                        fprintf(stderr, "Invalid exporter seed, expected 1..18446744073709551615\n");
                        return 1;
                    }
                    haveSeed = true;
                    i += 2;
                } else if (strcmp(argv[i + 1], "--depths") == 0 && i + 2 < argc) {
                    char *end = NULL;
                    unsigned long parsedDepths = strtoul(argv[i + 2], &end, 10);
                    if (end == argv[i + 2] || *end != '\0' || parsedDepths == 0 || parsedDepths > 40) {
                        fprintf(stderr, "Invalid exporter depth count, expected 1..40\n");
                        return 1;
                    }
                    depthCount = (unsigned int) parsedDepths;
                    haveDepthCount = true;
                    i += 2;
                } else {
                    fprintf(stderr, "Unexpected exporter argument: %s\n", argv[i + 1]);
                    return 1;
                }
            }

            if (!haveSeed || !haveDepthCount) {
                fprintf(stderr, "Exporter requires --seed and --depths\n");
                return 1;
            }

            currentConsole = nullConsole;
            errorMessage[0] = '\0';
            if (exportDungeonJson(seed, depthCount, outputPath, errorMessage) != 0) {
                fprintf(stderr, "Dungeon export failed: %s\n", errorMessage[0] ? errorMessage : "unknown error");
                return 1;
            }
            return 0;
        }
