...
class CreatureTests(unittest.TestCase):
    ...
    def test_verification_preserved_until_asset_hash_changes(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            baseline['creatures'][2]['verification']={'review':'accepted-for-test'}
            ...
            with patch.object(bestiary,'INDEX',target), patch.object(bestiary,'write_docs'):
                ...
                self.assertEqual(same['creatures'][2]['verification'],{'review':'accepted-for-test'})
                original=baseline['creatures'][2]['art']
                changed={key:original[key] for key in ('triangles','parts','bounds','objSha256','skinSha256')}
                changed['objSha256']='different-geometry'
                result=bestiary.write_index({'MK_KOBOLD':changed})
                self.assertTrue(result['creatures'][2]['verification']['stale'])
                self.assertEqual(result['creatures'][2]['verification']['previousAssetVerification'],
    ...
    def test_runtime_matches_deterministic_geometry_and_recorded_hashes(self):
        for record in self.records[2:]:
            with self.subTest(kind=record['kind']):
                ...
                model=(ROOT/entry['runtimeModel']).read_bytes()
                self.assertEqual(model,creatures.obj_bytes(creatures.build_parts(record),record['symbol']))
                self.assertEqual(hashlib.sha256(model).hexdigest(),entry['objSha256'])
                self.assertEqual(hashlib.sha256((ROOT/f"mod/BrogueDoom/graphics/BRGM{record['kind']:02d}.png").read_bytes()).hexdigest(),entry['skinSha256'])
    ...
    def test_registry_preserves_authority_and_identity(self):
        ...
        for expected,entry in zip(self.records,registry['monsters']):
            ...
            if expected['kind']>1:
                self.assertEqual(entry['skin'],f"graphics/BRGM{expected['kind']:02d}.png")
                self.assertEqual(entry['authoredDimensions'],profiles()[expected['symbol']]['dimensions'])
        ...
        self.assertEqual(bindings.count('Scale 1.0 1.0 1.0'),68)
